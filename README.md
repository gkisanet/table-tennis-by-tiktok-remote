...
### 1. 룰 선택 화면 (Rule Selection)
- 게임 시작 전 단식/복식 및 세트 수(3/5판) 선택 가능
- **직관적인 UI**: `<-` 빨간색 화살표 커서, 선택된 항목은 채워진 사각형으로 표시
- **자동 진행**: OK 버튼 누르면 다음 설정으로 자동 이동하며 빠른 설정 가능

### 2. 세트 히스토리 표시 (Set History)
- 메인 점수판 상단에 세트별 승패 기록을 원형 아이콘으로 표시
- 🟢 **초록색 원**: 해당 세트 승리
- 🔴 **빨간색 원**: 해당 세트 패배
- ⚫ **빈 원**: 아직 진행되지 않은 세트

### 3. 코트 체인지 알림 (Court Change)
- 단식 마지막 세트에서 5점 도달 시 "CHANGE COURT" 알림 표시
- OK 버튼을 누르면 **점수, 세트 스코어, 서브권, 히스토리**가 모두 자동으로 좌우 반전됨
